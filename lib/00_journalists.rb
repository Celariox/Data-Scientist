...
puts liste.length #Il est aussi possible d'utiliser les méthodes Count ou Size à la place de Length
puts

puts "Combien d'handle contiennent un numéro ? "
puts liste.select { |i| i[/\d+/] }.length
puts liste.grep(/[0-9]/).count
puts

puts "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite ?"
puts liste.count { |x| x.downcase.include?("aude")}
puts liste.select { |i| i.downcase["aude"] }
puts

puts "Combien commencent par une majuscule ? "
puts liste.count { |x| x[/^@\p{Upper}/] }
puts liste.grep(/^@\p{Upper}/).count
puts
